...
export class HandleMiddleware implements ICdsMiddleware {
    public async use(@Req() req: any, @Jwt() jwt: string, @Next() next: OnEventHandler): Promise<any> {
        ...
        const originalUrl = req.req.originalUrl;

        const parts = originalUrl.split("/");
        const firstPart = parts[1];
        if (firstPart === "manage") {
            this.checkRoleForManagePath(req, firstPart);
        }
    }
